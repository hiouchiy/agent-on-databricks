...
SCHEMA = "agent"
PRED_TABLE = f"{CATALOG}.{SCHEMA}.cancer_predictions"
...
VECTOR_INDEX_FULLNAME = f"{CATALOG}.{SCHEMA}.manuals_index"
...
# =========
# 1) sklearnの乳がんデータを読み込み（569行×30特徴量）
# =========
cancer = load_breast_cancer(as_frame=True)   # Xがpandas DataFrameで取れる  [oai_citation:1‡Scikit-learn](https://scikit-learn.org/stable/modules/generated/sklearn.datasets.load_breast_cancer.html?utm_source=chatgpt.com)
X = cancer.data.copy()                       # 30特徴量のみ（targetは使わない）

# =========
# 2) サンプリング（N_PATIENTSが569超ならreplace=True）
# =========
rng = np.random.default_rng(SEED)
replace = N_PATIENTS > len(X)
idx = rng.choice(len(X), size=N_PATIENTS, replace=replace)

df = X.iloc[idx].reset_index(drop=True)

# 列名はスペース入り -> underscore に変換（あなたのDeltaテーブル列名に合わせる）
df.columns = [c.replace(" ", "_") for c in df.columns]

# patient_id / created_at
df.insert(0, "patient_id", [f"P{str(i).zfill(6)}" for i in range(N_PATIENTS)])
df["created_at"] = pd.Timestamp.now()

# =========
# 3) Delta(UC) テーブルへ上書き保存
# =========
...
(
    sdf.write
      .mode("overwrite")
      .option("overwriteSchema", "true")   # スキーマ差分があっても上書きしたい場合に安全
      .format("delta")
      .saveAsTable(TABLE)
)
